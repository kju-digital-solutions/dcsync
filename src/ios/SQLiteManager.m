...
//                    }
                    ...
                    sqlite3_bind_int(compiledStatement, index, [value intValue]);
                }
...
-(int)update:(NSMutableDictionary *)dict into:(NSString *)table primaryKey:(NSString *)primaryKey{
    ...
    if(sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
        ...
        NSString *primaryVal = [data valueForKey:primaryKey];
        if (primaryVal != nil){
            NSArray *matches = [self find:@"*" from:table where:[NSString stringWithFormat:@"%@ = '%@'", primaryKey, primaryVal]];
            ...
            if (matches != nil && matches.count > 0) {
                ...
                updateSql = [updateSql substringFromIndex:1];
                sql = [NSString stringWithFormat:@"UPDATE %@ SET %@ WHERE %@='%@'",table, updateSql, primaryKey, primaryVal];
                [data removeObjectForKey:primaryKey];
                values = [data allValues];
